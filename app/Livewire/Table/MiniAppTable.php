...
class MiniAppTable extends TableView
{
    public function data()
    {
        $query = MiniApp::query()->where('user_id', auth()->user()->id);
        ...
        $search = $this->search;
        if ($search) {
            $query->whereLike(['judul', 'deskripsi'], $search);
        }
    ...
    public function columns(): array
    {
        return [
            ...
            RestfulButton::make('miniapp')->only(['edit', 'destroy'])->setCellAttributes(['style' => 'text-align: right; width: 2em;'])
        ];
    }
}